...
const ReverseLookup = ({ postcode = '' }) => {
    ...
    const searchPostalCode = () => {
      for (const city in data) {
        ...
        for (const district in districts) {
          if (districts[district] == postalCode) {
            // 找到對應的城市和區域
            setCityTown({ city, district })
            return
          }
        }
      }

      // 若找不到對應的城市和區域，設定結果為 null
      setCityTown(null)
    }
  ...
  return (
    <span>
      {cityTown && (
        <span>
          <span>{cityTown.city}</span>
          <span>{cityTown.district}</span>
        </span>
      )}
      {cityTown === null && <span>找不到對應的城市和區域。</span>}
    </span>
  )
}
